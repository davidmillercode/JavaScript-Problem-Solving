...
function score( dice ) {
    ...
    var multiplier = [1000, 200, 300, 400, 500, 600];
    dice.forEach(function(die){
        result[die - 1] += 1;
    });
    var mostOfAnyArr = result.filter(function(num) { return num >= 3;});
    var bonusLoc = result.indexOf(mostOfAnyArr[0]);
    if (bonusLoc != -1) {
        score += multiplier[bonusLoc];
        result[bonusLoc] -= 3;
    }
